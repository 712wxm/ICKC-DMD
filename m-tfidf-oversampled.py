...
def main():
    ...
    for top_n_class in [2, 4]:
        ...
        skf = StratifiedKFold(n_splits=5)#k=5,k-fold交叉验证2 4 8
        ...
        for item in ['GaussianNB', 'MLP', 'KNN', 'SVM', 'RF','DT','AdaBoost','GBDT','XGB']: #'AdaBoost','GBDT','XGB'
            file.write('\n###################')
            ...
            for index in range(0, 5):
                ...
                #         #XGB
                if item == 'XGB': clf = XGBClassifier().fit(train[index].iloc[:, 0:(numOfColumns-1)], train[index]['name'])
                #         #LGB
                #if item == 'LGB': clf = LGBMRegressor().fit(train[index].iloc[:, 0:(numOfColumns-1)], train[index]['name'])
            ...
            #画图
            # fpr, tpr, threshold = metrics.roc_curve(test[index], predicted)
